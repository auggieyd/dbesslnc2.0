...
2. Import the lncRNA gene into the **esslnc** table. Execute the data import code `/store/dbess.ipynb：Step2,3` When importing data,simply modify the corresponding file names,table names,and field names, 
   ...
   - **Step2.4: Import the mouse's essential lncRNA(from dbesslnc)**:import `dbesslnc_mouse.csv` to esslnc
   - **Step3.1: Import the mapped gene IDs(for lncRNA verified by CRISPR  )**
     ...
     details see code:`/store/dbess.ipynb：Step2.1`
...
4. Import transcript annotations into the **trans** table (lncRNA verified by CRISPR). 
   ...
   - Using the UID from the previous step,generated by`dbess.ipynb:Step4`,the`seq_xxx.bed`generated by`/match/gen_fa.ipynb`,and the`/match/lncRNA2.fasta` file,generate basic annotations for transcripts and import them into the database.
   - Map public database IDs using `lncbook_map.tsv` and `noncode_map.tsv`.
   - For detailed code,see`/store/dbess.ipynb:Step4`.

5. Import transcript annotations into the **trans** table(lncRNA associated with variants)

   - Using the UID from the previous step,generated by`dbess.ipynb:Step3.2`,`variants_mapping.txt`
...
Generate:`/clinvar_map/db/exon/disease_trans.csv`,Import this file by:`/store/dbess.ipynb:step4`
     Generate:`/clinvar_map/db/exon/non_crispr_lncRNA.csv`,Import this file by:`/store/dbess.ipynb:step4`
   - Extract sequences from NONCODE and LncBook databases and import them into the database.details see code:`/store/dbess.ipynb:step4`
...
6. Import CRISPR experiment record`exp_crispr.csv` into **exp_crispr** table.
   ...
   - Details see code:`/store/dbess.ipynb:Step6`
   - Group by  `exp_type`  and  `target_id`  columns, for groups with a count of 1, mark as 'cell-line specific' and update in the table; for groups with a count of 2-5, mark as 'common essential'; and for groups with a count greater than 5, mark as 'core essential'.Details see code:`/store/dbess.ipynb:Step6.1`

7. Import variants Mapping file into **lncrna_variant_mapping** table and variants file into the **variants** table.
   - Create a local mapping table for lncrna_variant_mapping, detail see code`/store/dbess.ipynb:step3.2`
   - Replace `noncode_id` ,` lncbook_id`, `chr`, `start`, `end`, `strand` in  `/clinvar_map/db/construct_map/unique_mapping.csv` with  `UID`    (Script:     `/clinvar_map/db/construct_map/get_map.py`  ).
     Generate:`/clinvar_map/db/construct_map/final_mapping.csv`
...
8. Import human lncRNA esspression file into **exp_profile** table
   ...
   - Create a local mapping table for  transcrispt `UID_trans.txt`, detail see code`/store/dbess.ipynb:step3.2`
